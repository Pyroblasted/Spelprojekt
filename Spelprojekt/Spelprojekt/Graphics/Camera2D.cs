...
namespace Spelprojekt.Graphics
{
    public interface ICamera2D
    {
        ...
        /// </returns>
        bool IsInView(Vector2 position, Vector2 size);
    }
    ...
    public class Camera2D : ICamera2D
    {
        ...
        /// </summary>
        public void Initialize(Entity focus)
        {
            this.Focus = focus;
            ...
            Scale = 1f;
            MoveSpeed = 4f;
            _position = Focus.location - Position;
        }
        ...
        public void Update(GameTime gameTime)
        {
            ...
            _position.X += (int)((Focus.location.X - Position.X) * MoveSpeed * delta);
            _position.Y += (int)((Focus.location.Y - Position.Y) * MoveSpeed * delta);
        ...
        /// </returns>
        public bool IsInView(Vector2 position, Vector2 size)
        {
            // If the object is not within the horizontal bounds of the screen
            if ((position.X + size.X) < (Position.X - Origin.X) || (position.X) > (Position.X + Origin.X))
                return false;

            // If the object is not within the vertical bounds of the screen
            if ((position.Y + size.Y) < (Position.Y - Origin.Y) || (position.Y) > (Position.Y + Origin.Y))
                return false;
